...
const PreventionSummaryYear = () => {
    ...
    const getSummary = async (year) => {
        const res = await api.get(`/api/prevention-monthlies/${year}`)

        const newBullets = res.data.bullets
                                .filter(bullet => bullet.division_id === 2)
                                .map(bullet => {
...
export default PreventionSummaryYear